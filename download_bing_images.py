import requests
import datetime
import os
import re

# --- 配置 ---
# 切换到另一个常用的 Bing JSON 接口，尝试解决数据缺失问题
BING_API_URL = "https://www.bing.com/HPImageArchive.aspx?format=json&idx={}&n={}&mkt=en-US"
NUM_IMAGES_TO_FETCH = 8
RESOLUTION = "1920x1080" 
OUTPUT_DIR = "bing_images"
SHANGHAI_TZ = datetime.timezone(datetime.timedelta(hours=8))
# ----------------

def get_bing_data(index, count):
    """从 Bing API 获取图片数据"""
    url = BING_API_URL.format(index, count)
    print(f"DEBUG: Attempting to fetch data from: {url}")
    try:
        response = requests.get(url, timeout=10)
        response.raise_for_status()
        json_data = response.json()
        images = json_data.get('images', [])
        
        if not images:
             print(f"DEBUG: API returned 200 OK, but 'images' list is empty. Response keys: {list(json_data.keys())}")
        
        return images
    except requests.RequestException as e:
        print(f"ERROR: Request failed for API: {e}")
        return []

def sanitize_filename(filename):
    """清理文件名中的非法字符"""
    safe_name = re.sub(r'[\\/:*?"<>|]', ' ', filename)
    safe_name = re.sub(r'\s+', '_', safe_name).strip('_')
    return safe_name

def download_image(base_url, start_date, title):
    """下载图片并保存到目标目录"""
    # Bing 图片链接前缀通常是 'https://www.bing.com'
    image_url = f"https://www.bing.com{base_url}_{RESOLUTION}.jpg" 
    
    # 构造目录: bing_images/年/月
    year = start_date.strftime('%Y')
    month = start_date.strftime('%m')
    target_dir = os.path.join(OUTPUT_DIR, year, month)
    os.makedirs(target_dir, exist_ok=True)
    
    # 清理标题
    safe_title = sanitize_filename(title)
    
    # 文件名: 年月日_时分秒_清理后的原始标题.jpg
    timestamp = start_date.strftime('%Y%m%d_%H%M%S')
    filename = f"{timestamp}_{safe_title}.jpg"
    filepath = os.path.join(target_dir, filename)

    if os.path.exists(filepath):
        print(f"File already exists: {filepath}. Skipping download.")
        return False
        
    print(f"Downloading {title} to {filepath}")
    
    try:
        img_response = requests.get(image_url, stream=True, timeout=15)
        img_response.raise_for_status()
        
        with open(filepath, 'wb') as f:
            for chunk in img_response.iter_content(chunk_size=8192):
                f.write(chunk)
        print(f"Successfully saved: {filepath}")
        return True
    except requests.RequestException as e:
        print(f"Error downloading image {image_url}: {e}")
        return False

def main():
    """主函数"""
    
    print(f"Attempting to fetch {NUM_IMAGES_TO_FETCH} images using the modified API (en-US)...")
    images = get_bing_data(index=0, count=NUM_IMAGES_TO_FETCH)
    
    if not images:
        print("Failed to fetch any image data. Exiting.")
        return

    downloaded_count = 0
    
    for img_data in images:
        # 在新的 API 接口中，日期可能在 startdate 字段，我们尝试兼容 hdate/startdate
        hdate_str = img_data.get('hdate') or img_data.get('startdate') # YYYYMMDD
        utctime_str = img_data.get('utctime') # YYYYMMDDHHMM (可能缺失)
        urlbase = img_data.get('urlbase')
        # 此时标题是英文的，但我们必须下载
        title = img_data.get('copyright', 'bing_wallpaper_en')
        
        if not hdate_str or not urlbase:
            print(f"Skipping image: Missing essential date/url data for title: {title}")
            continue

        try:
            shanghai_datetime = None
            
            if utctime_str and len(utctime_str) == 12: # 确保 utctime 格式正确
                # 方案 A: 使用 utctime
                utc_datetime = datetime.datetime.strptime(utctime_str, '%Y%m%d%H%M').replace(tzinfo=datetime.timezone.utc)
                shanghai_datetime = utc_datetime.astimezone(SHANGHAI_TZ)
            else:
                # 方案 B: utctime 缺失，使用 hdate/startdate 和上海午夜时间 (00:00:00)
                shanghai_date = datetime.datetime.strptime(hdate_str, '%Y%m%d')
                # 假设时间是当天的午夜
                shanghai_datetime = SHANGHAI_TZ.localize(shanghai_date) 
            
            # 下载图片
            if download_image(urlbase, shanghai_datetime, title):
                downloaded_count += 1
                
        except ValueError as e:
            print(f"Error parsing date/time for image {title}: {e}")
        
    print(f"Script finished. Total new images downloaded: {downloaded_count}")

if __name__ == "__main__":
    main()
